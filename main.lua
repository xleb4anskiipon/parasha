if game.Players.LocalPlayer:FindFirstChild("Key").Value = true then
...
local SilentAimSettings = {
    ...
    ClassName = "Universal Silent Aim -Kuchi, Supreme and Anonymous",
    ToggleKey = "RightAlt",
...
Esp.HealthHighColor = Color3.new(0, 255, 0)
Esp.HealthLowColor = Color3.new(255, 255, 255)

-- ui creating & handling
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/violin-suzutsuki/LinoriaLib/main/Library.lua"))()
Library:SetWatermark("Parasha.hub")

local Window = Library:CreateWindow({Title = 'UNIVERSAL | PARASHA.HUB', Center = true, AutoShow = true, TabPadding = 8, MenuFadeTime = 0.2})
...
local MiscellaneousBOX = GeneralTab:AddLeftTabbox("Misc")
local FieldOfViewBOX = GeneralTab:AddLeftTabbox("Field Of View") do
    local Main = FieldOfViewBOX:AddTab("Visuals")
    
    Main:AddToggle("Visible", {Text = "Show FOV Circle"}):AddColorPicker("Color", {Default = Color3.fromRGB(230, 230, 250)}):OnChanged(function()
    ...
    Main:AddToggle("MousePosition", {Text = "Show Silent Aim Target"}):AddColorPicker("MouseVisualizeColor", {Default = Color3.fromRGB(230, 230, 250)}):OnChanged(function()
...
local CreateConfigurationBOX = Config:AddLeftTabbox("Create Configuration") do 
...
local ESpBOX = Visuals:AddLeftTabbox("esp") do 
      local Main = ESpBOX:AddTab("esp")
    Main:AddToggle("espenabled", {Text = "esp enabled", Default = false}):OnChanged(function()
        Esp.Enabled = Toggles.espenabled.Value
    end)

    Main:AddToggle("boxenb", {Text = "box enabled"}):AddColorPicker("boxcolor", {Default = Color3.fromRGB(230, 230, 250)}):OnChanged(function()
        Esp.ShowBox = Toggles.boxenb.Value
    end)
    Options.boxcolor:OnChanged(function()
         Esp.BoxColor = Options.boxcolor.Value
    end)

     Main:AddToggle("namesenb", {Text = "names enabled"}):AddColorPicker("namecolor", {Default = Color3.fromRGB(230, 230, 250)}):OnChanged(function()
        Esp.ShowName = Toggles.namesenb.Value
    end)
    Options.namecolor:OnChanged(function()
         Esp.BoxColor = Options.namecolor.Value
    end)

    Main:AddToggle("healthenb", {Text = "health enabled", Default = false}):OnChanged(function()
        Esp.ShowHealth = Toggles.healthenb.Value
    end)

    Main:AddToggle("distenb", {Text = "distance enabled", Default = false}):OnChanged(function()
        Esp.ShowDistance = Toggles.distenb.Value
    end)

    Main:AddToggle("skeltenb", {Text = "skeleton enabled(Only r15)"}):AddColorPicker("skeltcolor", {Default = Color3.fromRGB(230, 230, 250)}):OnChanged(function()
        Esp.ShowSkeletons = Toggles.skeltenb.Value
    end)
    Options.skeltcolor:OnChanged(function()
          Esp.SkeletonsColor = Options.skeltcolor.Value
    end)

    Main:AddToggle("tracenb", {Text = "tracer enabled"}):AddColorPicker("traccolor", {Default = Color3.fromRGB(230, 230, 250)}):OnChanged(function()
        Esp.ShowTracer = Toggles.tracenb.Value
    end)
    Options.traccolor:OnChanged(function()
        Esp.TracerColor = Options.traccolor.Value
    end)
    Main:AddDropdown("tracerpos", {AllowNull = true, Text = "tracer position", Default = Esp.TracerPosition, Values = {"Top", "Middle", "Bottom"}}):OnChanged(function()
        Esp.TracerPosition = Options.tracerpos.Value
    end)

end

local SaveConfigurationBOX = Config:AddLeftTabbox("Save Configuration") do 
...
local LoadConfigurationBOX = Config:AddLeftTabbox("Load Configuration") do 
...
local UISettings = Config:AddRightTabbox("UI Settings") do
  local Main = UISettings:AddTab("UI Settings")
